...
inline string infix_to_suffix(const string& str) {
    ...
    stack<char> s;
    for(auto&& c: str) {
        if(isalpha(c)) {    // if is a symbol
            ret += c;
        } else if(c == '(') {
            s.push(c);  
        } else if(c == ')') {
            while(s.top() != '(') { // the string must be valid, so we dont have to check isEmpty
                ret += s.top();
                s.pop();
            }
            s.pop();
...
int calc(Node* root) {
    ...
    case '*': return calc(root->left) * calc(root->right);
    case '/': return calc(root->left) / calc(root->right);
    default: return symtab[root->symbol];
    }
}
...
int main() {
    ...
    for(int i=0; i<LIM; i++) {
        int j = LIM - 1;
        while(j >= 0 && exprTree[i][j] == ' ') j--; 
        if(j > -1) { // if this line is not empty
            lineCnt++;
            exprTree[i][j+1] = '\0';   
        } else break;
    }
